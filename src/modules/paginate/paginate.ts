...
const paginate = <T extends Document, U extends Model<U>>(schema: Schema<T>): void => {
  ...
  schema.static(
    'paginate',
    async function (filter: Record<string, any>, options: IOptions, collation?: ICollation): Promise<QueryResult> {
      let sort: string = '';
      if (options.sortBy) {
      ...
      let docsPromise =
        collation != null
          ? this.find(filter).collation(collation).sort(sort).skip(skip).limit(limit).select(project)
          : this.find(filter).sort(sort).skip(skip).limit(limit).select(project);
